...
class OeeResult:
    ...
    # Bağlam metrikleri (OEE'yi etkilemez; panoda künye/availability detayında gösterilir).
    loaded_qty: float = 0.0   # Σ yüklenen parça
    good_count: float = 0.0   # Σ iyi parça
    redo_count: float = 0.0   # Σ yeniden işlenen parça
    span_min: float = 0.0     # gözlem penceresi (dk)
    downtime_union_min: float = 0.0  # plansız duruş birleşimi (DOWNTIME∪MICROSTOP, dk)
...
def _quality_metrics(production: list[dict]) -> tuple[float, float, float, float, float]:
    ...
    (no-scrap → ≈%100). Ham sayımlar (loaded/redo/good) bağlam için ayrıca döndürülür.
    Doluluk kaybı Q'da değil; ayrı FILL_LOSS kanalındadır.
    """
    ...
    return (
        _clamp01((loaded - redo) / loaded),
        _clamp01(good / loaded),
        float(loaded),
        float(redo),
        float(good),
    )
...
def compute_oee(
    ...
    avail, span, downtime = availability_from_events(events)
    ...
    return OeeResult(
        ...
        loaded_qty=loaded, good_count=good, redo_count=redo,
        span_min=span, downtime_union_min=downtime,
    )
