...
Referans mantık: simulator/src/accuracy.py::extract_loss_tree. 6 kategori; görünür
kanallar doğrudan, gizli kanallar (FILL/SPEED) çıkarımla kestirilir.
...
VISIBLE = ("DOWNTIME", "MICROSTOP", "QUALITY_REDO", "QUALITY_SCRAP")
...
class LossEntry:
    ...
    parts: float = 0.0    # malzeme kaybı (FILL_LOSS/QUALITY_REDO/QUALITY_SCRAP)
...
def extract_loss_tree(
    ...
    # Görünür malzeme kayıpları: production sayımları.
    parts["QUALITY_REDO"] = float(sum(p["redo_count"] for p in production))
    parts["QUALITY_SCRAP"] = float(sum(p["scrap_count"] for p in production))
